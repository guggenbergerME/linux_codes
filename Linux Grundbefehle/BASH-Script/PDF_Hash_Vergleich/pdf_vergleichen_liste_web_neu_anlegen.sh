...
# Zeilen in urls.txt mit [Text] definieren Abschnitte und werden im Log gespeichert
...
while IFS= read -r URL; do
    ...
    # Abschnittsmarkierungen wie [Text] speichern
    if [[ "$URL" =~ ^\[.*\]$ ]]; then
        SECTION="$URL"
        continue
    fi
    ...
    if [ ! -s "$TMP_FILE" ]; then
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') – $SECTION – Fehler beim Download: $URL" >> "$LOGFILE"
        rm -f "$TMP_FILE"
        continue
    fi
